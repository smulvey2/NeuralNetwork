...
public class NNImpl {
    ...
    public int predict(Instance instance) {
        // TODO: add code here
    	double x = 0;
    	for(int i = 0; i < instance.attributes.size() - 1; i++) {
    	...
    	for(int i = 0; i < outputNodes.size(); i++) {
    		...
    		x += outputNodes.get(i).getOutput();
    	}
    		int idx = -1;
    		double max = 0.0;
    		for(int i = 0; i < outputNodes.size(); i++) {
    			outputNodes.get(i).adjustOutput(x);
    	...
    	for(int i = 0; i < maxEpoch; i++) {
    		...
    		for(Instance instance : trainingSet) {
    			...
    			for(int j = 0; j < outputNodes.size(); j++) {
    				...
    				x += outputNodes.get(j).getOutput();
    			}
    			for(int j = 0; j < outputNodes.size(); j++) {
    				outputNodes.get(j).adjustOutput(x);
    			}
    			for(int j = 0; j < outputNodes.size(); j++) {
    				outputNodes.get(j).calculateDelta(instance, j, outputNodes);
    			}
    			for(int j = 0; j < hiddenNodes.size(); j++) {
    				hiddenNodes.get(j).calculateDelta(instance, j, outputNodes);
    			}
    			for(int j = 0; j < hiddenNodes.size(); j++) {
    		...
    		total = total/trainingSet.size();
    		System.out.printf("Epoch: " + i + ", " + "Loss: %.3e\n", total);
    	}
    }
    ...
    private double loss(Instance instance) {
        ...
        for(int i = 0; i < outputNodes.size(); i++) {
        	total += instance.classValues.get(i) * Math.log(outputNodes.get(i).getOutput());
        }
        total = (-1) * total;
        return total;
        }
